<template>
	<view class="me">
		<uni-nav-bar :shadow="true" backState="2000" fontColor="#FFF" type="transparent">
			<view class="icon-setup" slot="left"><image class="setting" src="@/static/setup.png" mode="aspectFit" @tap="gotoLink('/subpages/setting/setting')" /></view>
		</uni-nav-bar>

		<uni-nav-bar backState="1000" :titleCenter="false" v-if="navBarShow">
			<view class="icon-setup" slot="left"><image class="setting" src="@/static/setup_b.png" mode="aspectFit" @tap="gotoLink('/subpages/setting/setting')" /></view>
		</uni-nav-bar>
			...
			<view class="user-info">
				...
				<view class="user-content" @tap="checkLogin">
					...
					<view class="user-text">
						...
						<view class="user-brief">{{ bio ? bio : '这个人有点懒，什么都没留下' }}</view>
					</view>
				</view>
						...
						<view class="feed-one" v-for="(item, index) in leftList" :key="index">
							<view @tap="gotoLink('/subpages/feedinfo/feedinfo?id=' + item.id)">
								...
								<view class="feed-info">
									...
									<!-- 分享 -->
									<view class="right" @tap.stop="openSheet(item.id)"><image src="@/static/more.png" class="micon-more" /></view>
								</view>
							</view>
						</view>
					</template>
...
import { findUser, getFeeds } from '@/common/http.api.js'
...
export default {
	...
	methods: {
		...
		async getInfos() {
			...
			let resa = await findUser({
				name: this.userInfo.name
			});
			if (resa.id) {
				this.avatar = !!resa.avatar ? resa.avatar.url : this.userInfo.avatar;
				this.bio = resa.bio;
			} else {
				this.$refs.login.openLogin();
				return;
