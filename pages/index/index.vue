			...
			<swiper-item class="swiper-item sns-news">
				<view v-for="(item, index) in newsList" :key="index">
					<navigator class="one-new" open-type="navigate" :url="'/subpages/newinfo/newinfo?id=' + item.id">
						...
						<view class="right"><image class="pic" mode="aspectFill" :src="item.cover" /></view>
					</navigator>
				</view>
			</swiper-item>
		</swiper>
...
// 接口
import { getAdvert, getFeeds, getNews } from '@/common/http.api.js';
// 引入 时间日期格式化显示函数
...
export default {
	...
	methods: {
		...
		async getFeedsList() {
			...
			const feedList = feeds.map(item => {
				return {
					...item,
					cover: this.BaseFileURL + item.images[0].file,
					avatar: item.user?.avatar?.url || '/static/nopic.png',
					name: item.user.name
				};
			});
