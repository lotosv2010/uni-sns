<template>
	<view>
		...
		<view class="tab-bar">
			...
			<view class="mview">
				...
				<view class="mbtn" @tap="sendLove">
					<image class="micon" v-if="type == 'info'" :src="oneInfoClone.has_like ? '/static/lover.png' : '/static/love.png'" mode="aspectFit" />
					...
					<image class="micon" v-if="type == 'feed'" :src="oneInfoClone.has_like ? '/static/lover.png' : '/static/love.png'" mode="aspectFit" />
		...
		<view class="commentBox" v-if="showCommentBox">
			...
			<textarea class="texta" placeholder-style="color:#888" placeholder="想说点什么..." cursor-spacing="150" :focus="true" :fixed="true" :value="cinput" @input="getInput" />
			<view class="btns">
...
import { getNewComments, getFeedComments, commentOneFeed, commentOneInfo, likeThisNew, unlikeThisNew } from '@/common/http.api.js';
export default {
	mixins: [feedMixin],
	props: {
		oneInfo: Object,
	...
	methods: {
		...
		async getCommentsList() {
			if (this.type === 'feed') {
				// 获取当前动态评论列表信息
				let res = await getFeedComments({
					id: this.oneInfo.id
				});
				this.commintsList = res.comments;
			} else {
				// 获取当前资讯评论列表信息
				let res = await getNewComments({
					id: this.oneInfo.id
				});
				this.commintsList = res.comments;
			}
